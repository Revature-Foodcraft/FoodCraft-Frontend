...
const DisplayRecipe: React.FC<SearchProp> = ({ searchQuery }) => {
    ...
    try {
      ...
      response.meals.forEach((meal: any) => {
        apiRecipes.push({
          ...
          rating: 3,
          source: "api",
          images: meal.strMealThumb ? [meal.strMealThumb] : []  // Add image from API
        });
      });
    ...
    <div style={{ position: "relative", height: "100%" }}>
      ...
      >
        ...
        const imageUrl = recipes?.images?.[0] || recipes?.pictures?.[0] || "https://via.placeholder.com/250x150.png?text=No+Image";
          ...
          return (
            <div className="card" key={recipes.PK} style={{ minWidth: "250px", maxWidth: "300px",  border: "7px solid #f6efe4", // added border
              borderRadius: "6px", // optional for a slightly rounded look
              overflow: "hidden"}}>
              <img
                src={imageUrl}
                alt={recipes.name}
  ...
  style={{ backgroundColor: "white", padding: "8px 0" }} // optional: beige background
>
  <Link
  ...
  >
    <h5 style={{ margin: 0 }}>{recipes.name}</h5>
                ...
                <div
                  ...
                  style={{
                    backgroundColor: "#f6efe4", // soft beige
                    padding: "12px",
                    borderRadius: "6px"
                  }}
                >
